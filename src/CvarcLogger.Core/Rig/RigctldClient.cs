...
/// "USB") then the passband in Hz (currently unused here); any line starting with "RPRT" followed
/// by a negative number is an error response.</summary>
public class RigctldClient : IRigControlService
{
    ...
    public async Task<RigPollResult> PollAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            return RigPollResult.Ok(freqMhz, rawMode, mappedMode, BandCalculator.FromFrequencyMhz(freqMhz));
        }
