...
/// local snapshot current with -- falls back to the bundled park list (a MauiAsset,
/// Resources\Raw\PotaParks.csv, ~93,500 parks worldwide, a one-time local snapshot) when offline or
/// the API is unreachable. Ported from the WPF app's identically-named service -- same logic, just
/// reading the bundled CSV via FileSystem.OpenAppPackageFileAsync instead of a plain file path next to
...
public class PotaParkLookupService
{
    ...
    // fresh instance every time Awards is opened) -- static so the 93,500-row CSV is still only ever
    // parsed once per app run, not once per page open.
    ...
    private static async Task<Dictionary<string, PotaParkInfo>> EnsureLoadedAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            using var stream = await FileSystem.OpenAppPackageFileAsync("PotaParks.csv").ConfigureAwait(false);
